...
public final class LiveSpeechRecognizer implements RecognitionListener {
    ...
    private SpeechRecognizer recognizer; private boolean running; private boolean restarting; private String detectedLanguage; private String forcedLanguage="auto";
    ...
    private void restartRecognizer(){
        ...
        handler.post(() -> {
            ...
            createRecognizer(); restarting=false; listenAgain();
        });
    }
    ...
    private void listenAgain() {
        ...
        i.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_COMPLETE_SILENCE_LENGTH_MILLIS, 500L); i.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_POSSIBLY_COMPLETE_SILENCE_LENGTH_MILLIS, 350L);
        ...
        try { recognizer.startListening(i); listener.onStatus("Слушаю…"); }
        catch(Exception e){ listener.onError("Не удалось запустить распознавание"); handler.postDelayed(this::restartRecognizer,180); }
    }
    public void stop() { running=false; restarting=false; handler.removeCallbacksAndMessages(null); if(recognizer!=null){ try{recognizer.cancel();}catch(Exception ignored){} try{recognizer.destroy();}catch(Exception ignored){} recognizer=null; } }
    ...
    @Override public void onError(int error) {
        if (!running) return;
        if(error==SpeechRecognizer.ERROR_CLIENT || error==SpeechRecognizer.ERROR_RECOGNIZER_BUSY){ restartRecognizer(); return; }
        ...
        handler.postDelayed(this::listenAgain,120);
    }
    @Override public void onResults(Bundle results) { emit(results, true); handler.postDelayed(this::listenAgain,100); }
