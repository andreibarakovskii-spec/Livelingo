...
public final class NeuralVoiceManager {
    ...
    public void speak(String text,String language,boolean finalChunk,int voiceProfile){
        if(closed||text==null||text.isBlank())return;final String clean=text.trim();final String lang=shortLang(language);boolean useKokoro="en".equals(lang)&&isInstalled();boolean useSupertonic=!useKokoro&&isMultilingualInstalled();if(!useKokoro&&!useSupertonic){fallback.speak(text,language,finalChunk,voiceProfile);return;}
        worker.execute(()->{if(closed)return;try{String remaining=clean;while(!remaining.isBlank()&&!closed){int cut=Math.min(180,remaining.length());if(cut<remaining.length()){int ws=remaining.lastIndexOf(' ',cut);if(ws>80)cut=ws;}String part=remaining.substring(0,cut).trim();remaining=remaining.substring(cut).trim();byte[] wav;if(useKokoro){SupertonicBridge.release();int sid=voiceProfile<=0?10:(voiceProfile>=2?3:6);wav=KokoroBridge.synthesize(modelDir().getAbsolutePath(),part,sid);}else{KokoroBridge.release();wav=SupertonicBridge.synthesize(multilingualDir().getAbsolutePath(),part,lang,voiceProfile);}if(wav==null||wav.length<64)throw new IllegalStateException("пустой аудиорезультат");playWav(wav);}scheduleRelease();}catch(Throwable e){speaking=false;postStatus("AI Voice: "+safe(e.getMessage())+" · резервный голос");fallback.speak(clean,language,finalChunk,voiceProfile);scheduleRelease();}});
    }
