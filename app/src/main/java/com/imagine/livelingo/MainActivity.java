...
public class MainActivity extends Activity implements SessionRuntime.Observer {
    ...
    @Override public void onFinalizedMeeting(String encryptedId,String report){
        runOnUiThread(()->{
            openedMeetingId=encryptedId;
            meetingResult.setText(report);
            meetingResultCard.setVisibility(View.VISIBLE);
    ...
    private void renderInsights(List<com.imagine.livelingo.business.MeetingInsightEngine.Insight> list){
        ...
        for(com.imagine.livelingo.business.MeetingInsightEngine.Insight i:list){
            String p="• ";
            if("decision".equals(i.type))p="✓ Решение: ";
            else if("action".equals(i.type))p="☐ Задача: ";
            else if("risk".equals(i.type))p="⚠ Риск: ";
            else if("question".equals(i.type))p="? Вопрос: ";
            else if("followup".equals(i.type))p="→ Следующий шаг: ";
    ...
    private void showPage(String page){
        livePage.setVisibility("live".equals(page)?View.VISIBLE:View.GONE);
        meetingsPage.setVisibility("meetings".equals(page)?View.VISIBLE:View.GONE);
        libraryPage.setVisibility("library".equals(page)?View.VISIBLE:View.GONE);
        profilePage.setVisibility("profile".equals(page)?View.VISIBLE:View.GONE);
        navLive.setAlpha("live".equals(page)?1f:.45f);
        navMeetings.setAlpha("meetings".equals(page)?1f:.45f);
        navLibrary.setAlpha("library".equals(page)?1f:.45f);
        navProfile.setAlpha("profile".equals(page)?1f:.45f);
    ...
    private void selectMode(String mode){
        currentMode=mode;
        modeTranslate.setAlpha("translate".equals(mode)?1f:.45f);
        modeConversation.setAlpha("conversation".equals(mode)?1f:.45f);
        modeMeeting.setAlpha("meeting".equals(mode)?1f:.45f);
    ...
    private void toggle(){
        ...
        if(!active){
            sourceText.setText("Слушаю…");
            translatedText.setText("Перевод появится здесь");
            String target=LanguageCatalog.codeForDisplay((String)targetSpinner.getSelectedItem());
            LiveLingoForegroundService.start(this,currentMode,manualInput,target);
        }else{
            LiveLingoForegroundService.stop(this);
        }
    }

    private void shareMeeting(){
        String report=meetingResult.getText().toString();
        if(report.isBlank()||openedMeetingId==null){Toast.makeText(this,"Сначала откройте защищённую встречу",Toast.LENGTH_SHORT).show();return;}
        Intent i=new Intent(Intent.ACTION_SEND);i.setType("text/plain");i.putExtra(Intent.EXTRA_TEXT,report);startActivity(Intent.createChooser(i,"Поделиться итогами встречи"));
    }

    private void shareTrace(){
        try{File f=trace.file();Uri uri=FileProvider.getUriForFile(this,getPackageName()+".fileprovider",f);Intent i=new Intent(Intent.ACTION_SEND);i.setType("text/plain");i.putExtra(Intent.EXTRA_STREAM,uri);i.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);startActivity(Intent.createChooser(i,"Отправить журнал LiveLingo"));}
        catch(Exception e){Toast.makeText(this,"Не удалось открыть журнал: "+e.getMessage(),Toast.LENGTH_LONG).show();}
    }
