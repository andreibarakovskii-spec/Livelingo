...
public final class OfflineSpeaker implements TextToSpeech.OnInitListener {
    ...
    public boolean selectOfflineVoice(String tag) {
        ...
        if (best != null) { tts.setVoice(best); tts.setSpeechRate(1.08f); return true; }
        return tts.setLanguage(wanted) >= TextToSpeech.LANG_AVAILABLE;
    }
    private void selectProfileVoice(String tag,VoiceProfile.Band band){
        if(!ready)return;
        Locale wanted=LanguageCatalog.localeFor(tag);
        List<Voice> voices=new ArrayList<>();
        for(Voice v:tts.getVoices()) if(!v.isNetworkConnectionRequired()&&v.getLocale().getLanguage().equals(wanted.getLanguage())) voices.add(v);
        voices.sort(Comparator.comparing(Voice::getName));
        if(!voices.isEmpty()){
            int idx=band==VoiceProfile.Band.LOW?0:(band==VoiceProfile.Band.HIGH?voices.size()-1:voices.size()/2);
            tts.setVoice(voices.get(Math.max(0,Math.min(idx,voices.size()-1))));
        } else tts.setLanguage(wanted);
        tts.setPitch(band==VoiceProfile.Band.LOW?0.88f:(band==VoiceProfile.Band.HIGH?1.12f:1.0f));
        tts.setSpeechRate(1.08f);
    }
    public void speak(String text, boolean finalChunk) { speak(text,finalChunk,targetTag,VoiceProfile.Band.NEUTRAL); }
    public synchronized void speak(String text,boolean finalChunk,String language,VoiceProfile.Band band) {
        if (!ready || text == null || text.isBlank()) return;
        if(language!=null&&!language.isBlank()) targetTag=language;
        selectProfileVoice(targetTag,band==null?VoiceProfile.Band.NEUTRAL:band);
