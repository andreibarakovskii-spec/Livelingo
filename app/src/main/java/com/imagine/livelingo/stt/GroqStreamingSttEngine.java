...
public final class GroqStreamingSttEngine implements SttEngine {
    ...
    @Override public synchronized void start(){if(running)return;if(!isAvailable()){listener.onError("Для непрерывного Groq Whisper нужен ключ gsk_…");return;}speakerIdentifier.reset();speakerIdentifier.ensureModelAsync(listener::onStatus);chunker=new SpeechChunker((samples,finalChunk)->{if(finalChunk&&running)network.execute(()->transcribe(samples));},listener::onSpeechStart);capture=new PcmAudioCapture(context,new PcmAudioCapture.Listener(){@Override public void onPcm(float[] samples){SpeechChunker c=chunker;if(running&&c!=null)c.accept(samples);}@Override public void onError(String message){listener.onError(message);}});running=true;listener.onStatus("Groq Whisper · непрерывная запись");capture.start();listener.onReady();}

    private void transcribe(float[] samples){
        if(!running||samples==null||samples.length==0)return;String key=keys.load();if(key==null||!key.startsWith("gsk_")){listener.onError("Groq API-ключ недоступен");return;}HttpURLConnection c=null;
        try{
            int speaker=speakerIdentifier.identify(samples,PcmAudioCapture.SAMPLE_RATE);if(speaker>0)listener.onSpeakerId(speaker);listener.onVoiceProfile(VoiceProfileAnalyzer.analyze(samples));
            byte[] wav=wav(samples);String boundary="----LiveLingo"+UUID.randomUUID().toString().replace("-","");c=(HttpURLConnection)new URL(ENDPOINT).openConnection();c.setRequestMethod("POST");c.setConnectTimeout(15_000);c.setReadTimeout(60_000);c.setDoOutput(true);c.setRequestProperty("Authorization","Bearer "+key);c.setRequestProperty("Content-Type","multipart/form-data; boundary="+boundary);
            ...
            int code=c.getResponseCode();String raw=read(code>=200&&code<300?c.getInputStream():c.getErrorStream());if(!running)return;if(code<200||code>=300){listener.onError("Groq Whisper HTTP "+code+": "+trim(raw));return;}JSONObject j=new JSONObject(raw);String text=j.optString("text","").trim();String language=j.optString("language","").trim();if(text.isBlank())return;listener.onFinal(text,language.isBlank()?forcedLanguage:language);
        }catch(Exception e){if(running)listener.onError("Groq Whisper: "+(e.getMessage()==null?e.getClass().getSimpleName():e.getMessage()));}finally{if(c!=null)c.disconnect();}
    }
