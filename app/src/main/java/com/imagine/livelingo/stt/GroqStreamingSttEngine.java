...
/**
 * Continuous microphone capture with cloud Whisper transcription through Groq.
 * AudioRecord stays open for the whole meeting; only completed VAD chunks are uploaded,
 * so Android SpeechRecognizer start/stop tones are never involved.
 */
public final class GroqStreamingSttEngine implements SttEngine {
    ...
    public GroqStreamingSttEngine(Context context,Listener listener){
        this.context=context.getApplicationContext();this.listener=listener;this.keys=new SecureApiKeyStore(this.context);
    }
    ...
    @Override public synchronized void start(){
        ...
        chunker=new SpeechChunker((samples,finalChunk)->{
            if(!finalChunk||!running)return;
            listener.onVoiceProfile(VoiceProfileAnalyzer.analyze(samples));
            network.execute(()->transcribe(samples));
        ...
        running=true;
        listener.onStatus("Groq Whisper · непрерывная запись");
        capture.start();
        listener.onReady();
    }
    ...
    private void transcribe(float[] samples){
        ...
        try{
            ...
            if(!running)return;
            if(code<200||code>=300){listener.onError("Groq Whisper HTTP "+code+": "+trim(raw));return;}
            JSONObject j=new JSONObject(raw);String text=j.optString("text","").trim();String language=j.optString("language","").trim();
    ...
    private static byte[] wav(float[] samples)throws Exception{
        ByteArrayOutputStream out=new ByteArrayOutputStream(44+samples.length*2);
        int data=samples.length*2,total=36+data,rate=PcmAudioCapture.SAMPLE_RATE,byteRate=rate*2;
        ascii(out,"RIFF");le32(out,total);ascii(out,"WAVEfmt ");le32(out,16);le16(out,1);le16(out,1);le32(out,rate);le32(out,byteRate);le16(out,2);le16(out,16);ascii(out,"data");le32(out,data);
        for(float f:samples){int v=Math.max(-32768,Math.min(32767,Math.round(f*32767f)));le16(out,v&0xffff);}
    ...
    @Override public synchronized void close(){stop();network.shutdownNow();}
}
