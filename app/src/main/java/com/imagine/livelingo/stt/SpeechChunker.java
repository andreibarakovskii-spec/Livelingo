package com.imagine.livelingo.stt;

import com.imagine.livelingo.DebugTrace;
import java.util.ArrayList;
import java.util.List;

/** Lightweight streaming VAD with generous pre-roll so first syllables are not clipped. */
public final class SpeechChunker {
    public interface Listener { void onChunk(float[] samples, boolean finalChunk); }
    private final Listener listener;
    private final Runnable speechStartListener;
    private final List<Float> speech=new ArrayList<>();
    private static final int SAMPLE_RATE=16000;
    private static final int PRE_ROLL=SAMPLE_RATE*65/100;
    private static final int END_SILENCE=SAMPLE_RATE*40/100;
    private static final int MIN_SPEECH=SAMPLE_RATE*18/100;
    private static final int PARTIAL_INTERVAL=SAMPLE_RATE*9/10;
    private final float[] preRoll=new float[PRE_ROLL];
    private int preWrite,preCount,silentSamples,voicedSamples,sincePartial;
    private boolean speaking;
    private float noiseFloor=0.008f;
    private long segmentStartedMs;

    public SpeechChunker(Listener listener){this(listener,null);}
    public SpeechChunker(Listener listener,Runnable speechStartListener){this.listener=listener;this.speechStartListener=speechStartListener;}

    public synchronized void accept(float[] pcm){
        if(pcm==null||pcm.length==0)return;
        double sum=0;float peak=0;for(float v:pcm){sum+=v*v;peak=Math.max(peak,Math.abs(v));}float rms=(float)Math.sqrt(sum/pcm.length);
        if(!speaking)noiseFloor=noiseFloor*.988f+rms*.012f;
        float startThreshold=Math.max(0.0105f,noiseFloor*2.05f);
        float holdThreshold=Math.max(0.0075f,noiseFloor*1.45f);
        boolean voice=rms>=(speaking?holdThreshold:startThreshold);

        if(!speaking){
            if(voice){
                speaking=true;segmentStartedMs=System.currentTimeMillis();silentSamples=0;voicedSamples=pcm.length;sincePartial=pcm.length;
                int preMs=preCount*1000/SAMPLE_RATE;appendPreRoll();clearPreRoll();append(pcm);
                DebugTrace.logGlobal("VAD_START","rms="+fmt(rms)+" peak="+fmt(peak)+" noise="+fmt(noiseFloor)+" threshold="+fmt(startThreshold)+" preroll_ms="+preMs);
                if(speechStartListener!=null)speechStartListener.run();
            }else pushPreRoll(pcm);
        }else if(voice){
            silentSamples=0;voicedSamples+=pcm.length;append(pcm);sincePartial+=pcm.length;
        }else{
            append(pcm);silentSamples+=pcm.length;sincePartial+=pcm.length;
        }

        if(speaking&&sincePartial>=PARTIAL_INTERVAL&&voicedSamples>=MIN_SPEECH){
            float[] chunk=copy();
            DebugTrace.logGlobal("VAD_PARTIAL","audio_ms="+(chunk.length*1000/SAMPLE_RATE)+" voiced_ms="+(voicedSamples*1000/SAMPLE_RATE));
            listener.onChunk(chunk,false);sincePartial=0;
        }
        if(speaking&&silentSamples>=END_SILENCE){
            if(voicedSamples>=MIN_SPEECH){float[] chunk=copy();DebugTrace.logGlobal("VAD_END","audio_ms="+(chunk.length*1000/SAMPLE_RATE)+" voiced_ms="+(voicedSamples*1000/SAMPLE_RATE)+" silence_ms="+(silentSamples*1000/SAMPLE_RATE)+" wall_ms="+(System.currentTimeMillis()-segmentStartedMs));listener.onChunk(chunk,true);}else DebugTrace.logGlobal("VAD_DROP","voiced_ms="+(voicedSamples*1000/SAMPLE_RATE));
            resetSpeech();
        }
    }

    public synchronized void flush(){if(speaking&&voicedSamples>=MIN_SPEECH){float[] chunk=copy();DebugTrace.logGlobal("VAD_FLUSH","audio_ms="+(chunk.length*1000/SAMPLE_RATE));listener.onChunk(chunk,true);}resetSpeech();clearPreRoll();}
    private void pushPreRoll(float[] pcm){for(float v:pcm){preRoll[preWrite]=v;preWrite=(preWrite+1)%preRoll.length;if(preCount<preRoll.length)preCount++;}}
    private void appendPreRoll(){if(preCount==0)return;int start=(preWrite-preCount+preRoll.length)%preRoll.length;for(int i=0;i<preCount;i++)speech.add(preRoll[(start+i)%preRoll.length]);}
    private void clearPreRoll(){preWrite=0;preCount=0;}
    private void append(float[] pcm){for(float v:pcm)speech.add(v);}
    private float[] copy(){float[] out=new float[speech.size()];for(int i=0;i<out.length;i++)out[i]=speech.get(i);return out;}
    private void resetSpeech(){speech.clear();silentSamples=0;voicedSamples=0;sincePartial=0;speaking=false;segmentStartedMs=0;}
    private static String fmt(float v){return String.format(java.util.Locale.US,"%.4f",v);}
}
