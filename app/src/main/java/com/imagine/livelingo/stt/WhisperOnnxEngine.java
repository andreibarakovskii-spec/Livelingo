...
public final class WhisperOnnxEngine implements SttEngine {
    ...
    @Override public synchronized void start(){
        if(running)return;if(!isAvailable()){listener.onError("Локальная модель LiveLingo AI ещё не установлена");return;}
        ...
        stabilizer.reset();speakerIdentifier.reset();speakerIdentifier.ensureModelAsync(listener::onStatus);
        chunker=new SpeechChunker((samples,finalChunk)->decodeExecutor.execute(()->decode(samples,finalChunk)),listener::onSpeechStart);
        capture=new PcmAudioCapture(context,new PcmAudioCapture.Listener(){@Override public void onPcm(float[] samples){SpeechChunker c=chunker;if(running&&c!=null)c.accept(samples);}@Override public void onError(String message){listener.onError(message);}});
    ...
    private void decode(float[] samples,boolean finalChunk){
        ...
        try{
            WhisperOnnxDecoder.Result r=decoder.transcribe(samples,forcedLanguage);if(r==null||r.text==null||r.text.isBlank())return;
            String stable=stabilizer.accept(r.text,finalChunk);if(stable.isBlank())return;
            if(finalChunk){listener.onVoiceProfile(VoiceProfileAnalyzer.analyze(samples));int speaker=speakerIdentifier.identify(samples,PcmAudioCapture.SAMPLE_RATE);if(speaker>0)listener.onSpeakerId(speaker);listener.onFinal(stable,r.language);}else listener.onPartial(stable,r.language);
